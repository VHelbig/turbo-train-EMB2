...
void hwmm_layer1(volatile uint8_t input[100], const float weights[100][32], float output[1][32]);
...
__attribute__((sdx_kernel("nn_inference", 0))) void nn_inference(volatile uint8_t input_img[100],volatile uint8_t output_bus[1],volatile uint8_t *out_wire);
...
void hwmm_layer1(volatile uint8_t input[100], const float weights[100][32], float output[1][32]) {
 layer_1_col: for (int j = 0; j < 32; ++j) {
#pragma HLS UNROLL
 float sum = 0;

      layer_1_prod: for (int k = 0; k < 100; ++k){
...
void hwmm_layer2(float input[1][32], const float weights[32][16], float output[1][16]) {
 ...
 layer_2_col: for (int j = 0; j < 16; ++j) {

      float sum = 0;

      layer_2_prod: for (int k = 0; k < 32; ++k){
...
void hwmm_layer3(float input[1][16], const float weights[16][10], float output[1][10]) {
    ...
    layer_3_col: for (int j = 0; j < 10; ++j) {

      float sum = 0;

      layer_3_prod: for (int k = 0; k < 16; ++k){
...
void hw_act_layer1(float input[1][32], float output[1][32]){
 layer_1_act: for (int i = 0; i < 32; i++){
...
void hw_act_layer2(float input[1][16], float output[1][16]){
 layer_2_act: for (int i = 0; i < 16; i++){
...
void hw_act_layer3(float input[1][10], int &pred){
 int max_idx = -1;
 float max_val = -1;
 layer_3_act: for (int i = 0; i < 10; i++){
...
__attribute__((sdx_kernel("nn_inference", 0))) void nn_inference(volatile uint8_t input_img[100],volatile uint8_t output_bus[1],volatile uint8_t *out_wire){_ssdm_SpecArrayDimSize(input_img, 100);_ssdm_SpecArrayDimSize(output_bus, 1);
...
#pragma HLS INTERFACE ap_ovld port=out_wire
 ...
 output_bus[0]=(uint8_t)prediction;
 *out_wire=(uint8_t)prediction;
