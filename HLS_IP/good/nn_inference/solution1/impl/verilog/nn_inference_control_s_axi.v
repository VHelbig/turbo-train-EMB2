...
// ==============================================================
`timescale 1ns/1ps
module nn_inference_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_OUTPUT_BUS_DATA_0 = 6'h1c,
    ADDR_OUTPUT_BUS_DATA_1 = 6'h20,
    ADDR_OUTPUT_BUS_CTRL   = 6'h24,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_OUTPUT_BUS_DATA_0: begin
                    rdata <= int_output_bus[31:0];
                end
                ADDR_OUTPUT_BUS_DATA_1: begin
                    rdata <= int_output_bus[63:32];
                end
            endcase
        end
    end
...
assign output_bus = int_output_bus;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_output_bus[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_BUS_DATA_0)
            int_output_bus[31:0] <= (WDATA[31:0] & wmask) | (int_output_bus[31:0] & ~wmask);
    end
end

// int_output_bus[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_bus[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_BUS_DATA_1)
            int_output_bus[63:32] <= (WDATA[31:0] & wmask) | (int_output_bus[63:32] & ~wmask);
    end
end
